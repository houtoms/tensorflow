  ...
  def apply_gradients(self, grads_and_vars, global_step=None, name=None):
    ...
    def skip_update():
      if global_step is not None:
        return array_ops.identity(global_step, name=name+'-skip').op
      else:
        return gen_control_flow_ops.no_op(name=name+'-skip')

    with ops.control_dependencies([shift_update_op]):
      cond = control_flow_ops.cond(all_finite, do_update, skip_update, name='skip_check')

    if global_step is not None:
      graph = ops.get_default_graph()
      current_scope = graph.get_name_scope()
