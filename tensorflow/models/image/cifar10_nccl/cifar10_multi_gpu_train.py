...
def train():
  ...
  with tf.Graph().as_default(), tf.device('/cpu:0'):
    ...
    for i in xrange(FLAGS.num_gpus):
      with tf.device('/gpu:%d' % i):
        with tf.name_scope('%s_%d' % (cifar10.TOWER_NAME, i)) as scope:
          ...
          # all towers.
          with tf.variable_scope('var_%s_%d' % (cifar10.TOWER_NAME, i)) as vs:
    ...
    apply_gradient_op = opt.apply_gradients([g for grads in tower_grads for g in grads], global_step = global_step)
