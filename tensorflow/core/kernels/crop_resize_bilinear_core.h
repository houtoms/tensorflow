...
void crop_resize_single_image(
    ...
    float* output) TF_ATTRIBUTE_NOINLINE;
template <typename T>
void crop_resize_single_image(
    const T* image,
    const int64 in_height, const int64 in_width,
  ...
  if (min_iy > 0) {
    ...
    float* p = output;
    for (int64 i = 0;  i < nn;  ++i) p[i] = extrapolated_value;
  }
  // high y extrapolation zone
  if (max_iy < out_height-1) {
    int64 ny = (int64)(out_height-1-max_iy);
    int64 nn = out_row_size * ny;
    float* p = output + out_row_size * (int64)(max_iy+1);
    for (int64 i = 0;  i < nn;  ++i) p[i] = extrapolated_value;
  }
  ...
  // interpolation zone
  float* output_y_ptr = output + out_row_size * (int64)min_iy;
  if (channels == 1) {
	...
	output_y_ptr[x*4 + 2] =
		compute_lerp(top_left2, top_right2, bottom_left2, bottom_right2,
				xs_lerp, ys_lerp);
	output_y_ptr[x*4 + 3] =
		compute_lerp(top_left3, top_right3, bottom_left3, bottom_right3,
				xs_lerp, ys_lerp);
      }
      output_y_ptr += out_row_size;
    }
