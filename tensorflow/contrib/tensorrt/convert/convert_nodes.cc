...
tensorflow::Status ConvertRelu6(
  ...
  // TRT only uses dynamic ranges in INT8 precision mode, and this does not
  // affect the FP32 path.
  ...
  for (int i = 0; i < dims.nbDims; i++)
    dims.d[i] = 1;
  TRT_ShapedWeights weights = ctx.get_temp_weights(
      tensorflow::DataType::DT_FLOAT, dims);
  auto weights_ptr = static_cast<float*>(const_cast<void*>(
      weights.GetValues()));
  weights_ptr[0] = 6.f;
  nvinfer1::IConstantLayer* const6_layer = ctx.network()->addConstant(
      dims, weights);
