...
def _CreateParamsSavable(params,
  ...
  """
  if model._rnn_mode == CUDNN_LSTM: # pylint:disable=protected-access
    fn = cudnn_rnn_ops.CudnnLSTMSaveable
  elif model._rnn_mode == CUDNN_GRU: # pylint:disable=protected-access
    fn = cudnn_rnn_ops.CudnnGRUSaveable
  elif model._rnn_mode == CUDNN_RNN_TANH: # pylint:disable=protected-access
    fn = cudnn_rnn_ops.CudnnRNNTanhSaveable
  elif model._rnn_mode == CUDNN_RNN_RELU: # pylint:disable=protected-access
...
class CudnnRNNTestSaveRestore(TensorFlowTestCase):
  ...
  def _testSaveRestoreVariable(self, rnn_mode, direction, dtype):
    ...
    with ops.Graph().as_default():
      ...
      weights, biases = saveable._OpaqueParamsToCanonical() # pylint:disable=protected-access
  ...
  def _testSaveRestoreTwoVariables(self, rnn_mode, direction, dtype):
    ...
    with ops.Graph().as_default():
      ...
      weights1, biases1 = saveables[0]._OpaqueParamsToCanonical() # pylint:disable=protected-access
      weights2, biases2 = saveables[1]._OpaqueParamsToCanonical() # pylint:disable=protected-access
      reset_params = [
          state_ops.assign(
              params,
              array_ops.zeros([params_size_t], dtype=dtype),
  ...
  def _createCheatSheet(self, rnn_mode, num_layers, num_units, input_size,
    ...
    for seq_len in range(seq_length):
      ...
      with self.test_session(use_gpu=True, graph=ops.get_default_graph()) \
        ...
        sess.run(variables.global_variables_initializer())
        if has_input_c:
          o_sum_, oh_sum_, oc_sum_ = sess.run([output_sum, output_h_sum,
                                               output_c_sum])
  ...
  def _testOneSimpleInference(self, rnn_mode, num_layers, num_units, input_size,
    ...
    with self.test_session(use_gpu=True, graph=ops.get_default_graph()) as sess:
      ...
      if has_input_c:
        ...
        o_sum_, oh_sum_ = sess.run([output_sum, output_h_sum])
