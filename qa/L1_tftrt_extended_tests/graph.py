...
def get_preprocess_fn(model, mode='classification'):
    ...
    def process(record):
        ...
        image = NETS[model].preprocess(image, NETS[model].input_height, NETS[model].input_width, is_training=False)
        return image, label
...
def build_classification_graph(model):
    ...
    with tf.Graph().as_default() as tf_graph:
        with tf.Session(config=tf_config) as tf_sess:
            tf_input = tf.placeholder(tf.float32, [None, net.input_height, net.input_width, 3],name='input')
            if net.slim:
                # TF Slim Model: get model function from nets_factory
                network_fn = nets.nets_factory.get_network_fn(net.name, net.num_classes,
                        is_training=False)
