...
TARGETS="tests(//tensorflow/python/kernel_tests:*)"
...
cd $SCRIPT_DIR/../..
bazel query "attr(size, small, $TARGETS) except (attr(tags, no_gpu, $TARGETS) $ARM_EXCEPT union attr(data, '/*\.so', $TARGETS))" > "$TEST_LIST"
...
for i in $(seq 0 $((GPUS-1))); do
  {
    ...
    while read -r TEST; do
      TEST_NAME=${TEST##*:}
      TEST_FILE="tensorflow/python/kernel_tests/$TEST_NAME.py"
