...
def benchmark(n_gpus, use_tf_amp, use_xla):
    ...
    baseline_dir = os.path.join(base_dir, "benchmark_baselines/trainbench")

    performance_file = "/tmp/performances_train.json"
    ...
    for class_id in range(1, 11):
        ...
        print(
            "[Running] QA Job - UNet TF Industrial - Training Benchmark\n"
            "\t[*] N GPUs: %d\n"
            "\t[*] Use TF AMP: %s\n"
            "\t[*] Use XLA: %s\n"
            "\t[*] DAGM Class ID: %d" %
        ...
        baseline_file = os.path.join(
            baseline_dir,
            "UNet_Industrial_%dGPU_%s.json" % (n_gpus, "AMP" if use_tf_amp else "FP32")
        )
