...
def benchmark(n_gpus, use_tf_amp, use_xla):
    ...
    gpu_batch_size = int(16 / n_gpus)

    base_dir = os.path.dirname(os.path.realpath(__file__))
    baseline_dir = os.path.join(base_dir, "benchmark_baselines")
    ...
    for class_id in range(1, 11):
        ...
        if n_gpus > 1:
            ...
            front_cmd = front_cmd + """
                mpirun \\
                    ...
                    --allow-run-as-root \\""" % (n_gpus, n_gpus)
        ...
        exec_command = front_cmd + """
            python %s/main.py \\
                    ...
                    --display_every=250 \\
                    --debug_verbosity=0
        """ % (unet_dir, gpu_batch_size, class_id, amp_flag, xla_flag)

        print(
            "[Running] QA Job - UNet TF Industrial - Convergence Test\n"
            "\t[*] N GPUs: %d\n"
            "\t[*] Use TF AMP: %s\n"
            "\t[*] Use XLA: %s\n"
            "\t[*] DAGM Class ID: %d" %
            (n_gpus, use_tf_amp, use_xla, class_id)
        )
        ...
        baseline_file = os.path.join(
            baseline_dir,
            "amp" if use_tf_amp else "fp32",
            "UNet_Industrial_DAGM_Class_%02d.json" % class_id
        )
        ...
        try:
            ...
            for metric_key, metric_dict in baseline_dict.items():

                for threshold, val in metric_dict.items():

                    if float(results_dict[metric_key][threshold]) < float(val):
                        raise RuntimeError(
                            "[Error] The model did not converged to the target baseline.\n"
                            "HParams:\n"
                            "\t[*] N GPUs: %s\n"
                            "\t[*] Use TF AMP: %s\n"
                            "\t[*] Use XLA: %s\n"
...
if __name__ == "__main__":
    ...
    successes, failures = benchmark(n_gpus=FLAGS.n_gpus, use_tf_amp=FLAGS.use_tf_amp, use_xla=FLAGS.use_xla)
