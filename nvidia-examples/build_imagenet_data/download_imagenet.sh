...
BASE_URL="http://www.image-net.org/challenges/LSVRC/2012"
echo "Checking download URL at ${BASE_URL}"
if [[ `wget --timeout=10 --tries=3 -S --spider ${BASE_URL}/nnoupb/${BBOX_FILENAME} 2>&1 | grep '200 OK'` ]]; then
  BASE_URL="${BASE_URL}/nnoupb"
  echo "Alternate URL looks good"
elif [[ `wget --timeout=10 --tries=3 -S --spider "${BASE_URL}/nonpub/${BBOX_FILENAME}" 2>&1 | grep '200 OK'` ]]; then
  BASE_URL="${BASE_URL}/nonpub"
  echo "Original URL looks good"
else
  wget --timeout=10 --tries=1 -S --spider ${BASE_URL}/nnoupb/${BBOX_FILENAME}
  echo "ERROR: URL did not work"
fi
